...
function init() {
  ...
  // CAMERA
  camera = new THREE.PerspectiveCamera(450, width/height, 0.1, 25000);
...
function animate() {
  ...
  let timer = date.getTime() * 0.0005; // get time string, changing speed
  camera.position.y = 5 * Math.cos(timer);
  camera.position.x = 1 * Math.cos(timer); // multiplier changes X coordinate
