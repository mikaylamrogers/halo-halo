...
function init() {
  ...
  // CAMERA
  camera = new THREE.PerspectiveCamera(740, width/height, 0.1, 50);
  camera.position.set(10, 10, 80);
  scene.add(camera);

  // LIGHTING
  let spotlight = new THREE.SpotLight(0xffffff, 0.2, 1000); // color, intensity, distance
...
function animate() {
  ...
  camera.position.x = 70 * Math.cos(timer); // multiplier changes X coordinate
  camera.position.z = 40 * Math.sin(timer); // multiplier changes Z coordinate
